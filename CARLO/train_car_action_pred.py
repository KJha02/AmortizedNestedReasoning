from car_utils.car_data import ReasoningAboutScenario1L0Dataset, car_collate, joint_sa_tensor_to_state_action
...
def get_args_parser():
    ...
    parser.add_argument("--env-name", type=str, default="scenario1", help="Environment name")
...
def init_actionPred(args, device='cpu'):
...
def main(args):
    ...
    # you have to pass in the path to your state recognition models
    ...
    L0_model_save_dir = f"{BIG_STORAGE_DIR}CARLO/{args.save_dir}/{args.env_name}/{args.experiment_name}/{get_config_name(args)}"
    p = Path(L0_model_save_dir)
    if not p.is_dir():
            p.mkdir(parents=True)
    dataset_dir = f"{BIG_STORAGE_DIR}data/{args.env_name}"

    train_dataset = ReasoningAboutScenario1L0Dataset(
        beta=args.beta,
    ...
    test_dataset = ReasoningAboutScenario1L0Dataset(
        beta=args.beta,
    ...
    for epoch_id in tqdm(range(args.num_epochs)):
        ...
        if avgAccuracy > max_test_acc:
            ...
            save_checkpoint(f"{L0_model_save_dir}/checkpoints/best_acc.pik", model, optimizer, stats, args)

        if avgLoss < min_test_loss:
              min_test_loss = avgLoss
              save_checkpoint(f"{L0_model_save_dir}/checkpoints/best_loss.pik", model, optimizer, stats, args)

        if epoch_id % 10 == 0:
            print(f"Avg Train Loss Epoch {epoch_id}: {stats['train_losses'][-1]} ; Avg Test Loss Epoch {epoch_id}: {stats['test_losses'][-1]}; Avg Train Accuracy Epoch {epoch_id}: {stats['train_accuracies'][-1]}")
            ...
            ax.set_xlabel('Epoch')
            ax.set_ylabel('Loss')
            ax.set_title('NNL0 KL Divergence Loss')
            ax.grid(True)
            ...
            ax.set_title('NNL2 Goal Prediction Accuracy')
            ax.grid(True)

            car_utils.general.save_fig(fig, f"{L0_model_save_dir}/stats.png")
